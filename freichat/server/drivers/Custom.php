...
//------------------------------------------------------------------------------
    //AVATAR_URL_START
        public function avatar_url($res) {
            $root = 'http://localhost:81/message.php';
            $avatar = $res[$this->avatar_field_name];
            $avatar = str_replace(' ','%20',$avatar);

      $murl = "pictures/";$avatar_url = $murl . $avatar; 
      return $avatar_url; 
}
        //AVATAR_URL_END
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
    public function get_guests() {
        ...
        //CUSTOM_GUESTS_QUERY_START
            $query = "SELECT DISTINCT f.status_mesg,f.username,f.session_id,f.status,f.guest,f.in_room,a.image
                   FROM frei_session AS f LEFT JOIN logins AS a ON f.session_id = a.UserID
